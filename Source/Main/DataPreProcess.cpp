...
bool DataPreProcess::DPP_File_Load()
{
	...
	*/
	...
	// elements 가져오기
	{
		fopenResult = fopen_s( &fp_dataInfo, this->m_WeatherData_Info_Addr, "r" );
		...
		*/
		...
		{
			::gLog_ErrorLog.LS_AddLog( _T( "Invalid input format in WeatherData_Info file" ) );
	...
	fopenResult = fopen_s( &fp_fileList, this->m_WeatherData_FileList_Addr, "r" );
	errnoCheck = Errno_Checker( fopenResult );
	if( errnoCheck == false )
	{
		// 파일 오픈 실패
	...
	while( fscanf_s( fp_fileList, "%s\n", c_dataFileAddress, sizeof(c_dataFileAddress) ) != EOF )
	{
		...
		while( fscanf_s( fp_weatherData, "%s\n", c_weatherData, sizeof(c_weatherData) ) != EOF )
		{
			...
			while( c_weatherData[ findIndex ] != '\0' )
			{
				...
				{
					::gLog_ErrorLog.LS_AddLog( _T( "Invalid value input in WeatherData file ( .txt or .csv )" ) );
			...
			for( int readDataCnt = 0; readDataCnt < this->m_weatherElementsCnt; readDataCnt++ )
			{
				if( readPosition == nullptr )
				{
					::gLog_ErrorLog.LS_AddLog( _T( "Weather data elements is lacking in WeatherData file" ) );
...
bool DataPreProcess::DPP_File_Close( FILE *fp_filePtr )
{
	...
	{
		::gLog_ErrorLog.LS_AddLog( _T( "File close fail - DataPreProcess.cpp" ) );
...
bool DataPreProcess::DPP_MinMax_Scaling( int minRange, int maxRange )
{
	...
	*/
	...
	{
		::gLog_ErrorLog.LS_AddLog( _T( "Min value is larger than max value in MinMaxScaler" ) );
...
bool DataPreProcess::DPP_Train_Test_Split( E_Split_Shuffle shuffleCheck, float trainRatio, float testRatio, float ***trainX, float **trainY, float ***testX, float **testY )
{
	...
	if( trainRatio <= 0 || testRatio < 0 )
	{
		...
		::gLog_ErrorLog.LS_AddLog( _T( "Train ratio or test ratio is invalid value in Train-Test split" ) );
