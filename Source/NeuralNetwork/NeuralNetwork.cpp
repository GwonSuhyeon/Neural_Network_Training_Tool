...
bool NeuralNetwork::NN_Init_LossFunction( E_LossFunctionType lossFunction )
{
	switch( lossFunction )
	{
	case E_MSE:
	...
	this->m_LossFunctionType = lossFunction;
...
bool NeuralNetwork::NN_Init_Optimizer( int BatchSize, E_OptimizerType optimizer, int trainDataCnt )
{
	...
	case E_BGD:
	{
		if( BatchSize != trainDataCnt )
		{
			...
			::gLog_ErrorLog.LS_AddLog( _T( "Invalid batch size input in BGD optimizer" ) );
	...
	case E_SGD:
	{
		if( BatchSize != 1 )
		{
			...
			::gLog_ErrorLog.LS_AddLog( _T( "Invalid batch size input in SGD optimizer" ) );
	...
	case E_MGD:
	{
		if( BatchSize <= 1 || BatchSize >= trainDataCnt )
		{
			...
			::gLog_ErrorLog.LS_AddLog( _T( "Invalid batch size input in MGD optimizer" ) );
	...
	this->m_OptimizerType = optimizer;
...
E_LossFunctionType NeuralNetwork::NN_Get_LossFunction_Type()
{
	return this->m_LossFunctionType;
}


E_OptimizerType NeuralNetwork::NN_Get_Optimizer_Type()
{
	return this->m_OptimizerType;
}


bool NeuralNetwork::NN_DeletE_LossFunctionType()
{
	if( this->m_LossFunction == nullptr )
	{
		return false;
	...
	switch( this->m_LossFunctionType )
	{
	case E_MSE:
	{
...
bool NeuralNetwork::NN_DeletE_OptimizerType()
{
	if( this->m_Optimizer == nullptr )
	{
		return false;
...
bool NeuralNetwork::NN_Model_Learn( int Epoch, int BatchSize, float LearningRate, E_OptimizerType optimizer, E_LossFunctionType lossFunction, float **trainX, float *trainY, int trainDataCnt, float targetMin, float targetMax, int *LearnedDataCnt )
{
...
*/
	...
	if( Epoch <= 0 || BatchSize <= 0 || BatchSize > trainDataCnt )
	{
		...
		::gLog_ErrorLog.LS_AddLog( _T( "Epoch or batch size is invalid value - NeuralNetwork.cpp : NN_Model_Learn" ) );
	...
	{
		// 학습데이터를 잘못 넘어옴.
		::gLog_ErrorLog.LS_AddLog( _T( "TrainX or TrainY is nullptr - NeuralNetwork.cpp : NN_Model_Learn" ) );
	...
	for( int repeat = 0; repeat < Epoch; repeat++ )
	{
		...
		for( int dataIndex = 0; dataIndex < trainDataCnt; dataIndex++ )
		{
			...
			if( propResult == false )
			{
				::gLog_ErrorLog.LS_AddLog( _T( "Model foreward propagation fail - NeuralNetwork.cpp : NN_Model_Learn" ) );
			...
			if( lossResult == false )
			{
				::gLog_ErrorLog.LS_AddLog( _T( "Model error compute fail - NeuralNetwork.cpp : NN_Model_Learn" ) );
			...
			if( propResult == false )
			{
				::gLog_ErrorLog.LS_AddLog( _T( "Model backward propagation fail - NeuralNetwork.cpp : NN_Model_Learn" ) );
			...
			if( dataIndex == backwardStep )
			{
				...
				if( lossResult == false )
				{
					::gLog_ErrorLog.LS_AddLog( _T( "Model loss compute fail - NeuralNetwork.cpp : NN_Model_Learn" ) );
				...
				for( int i = 0; i < (int)Model.size() - 1; i++ )
				{
					...
					for( int gradientIndex = 0; gradientIndex < gradientWeighttSize; gradientIndex++ )
					{
						...
						{
							::gLog_ErrorLog.LS_AddLog( _T( "Model weight optimizing fail - NeuralNetwork.cpp : NN_Model_Learn" ) );
					...
					for( int gradientIndex = 0; gradientIndex < gradientBiasSize; gradientIndex++ )
					{
						...
						if( optimizingResult == false )
						{
							::gLog_ErrorLog.LS_AddLog( _T( "Model bias optimizing fail - NeuralNetwork.cpp : NN_Model_Learn" ) );
				...
				if( progressResult == false )
				{
					::gLog_ErrorLog.LS_AddLog( _T( "Model learning progress print fail - NeuralNetwork.cpp : NN_Model_Learn" ) );
...
bool NeuralNetwork::NN_Model_Predict( float **testX, float *testY, int testDataCnt )
{
...
bool NeuralNetwork::NN_Model_Save()
{
	FILE				*fp_ArchitectureFile	= nullptr;
	FILE				*fp_WeightFile			= nullptr;
	FILE				*fp_BiasFile			= nullptr;
	...
	E_ActivationType	activationFunctionType;
	E_LossFunctionType	lossFunctionType;
	E_OptimizerType		optimizerType;
	int					neuronSize;
	int					weightSize;
	int					biasSize;
	...
	char				architectureSaveAddr[ 512 ];
	char				weightSaveAddr[ 512 ];
	char				biasSaveAddr[ 512 ];


	if( this->m_modelSave_Directory_Addr == nullptr )
	{
		return false;
	}
	...
	strcpy_s( architectureSaveAddr, sizeof( architectureSaveAddr ), this->m_modelSave_Directory_Addr );
	strcat_s( architectureSaveAddr, "/" );
	strcat_s( architectureSaveAddr, "Model_Architecture.csv" );

	fopenResult = fopen_s( &fp_ArchitectureFile, architectureSaveAddr, "w" );
	errnoCheck = Errno_Checker( fopenResult );
	if( errnoCheck == false )
	{
		// 파일 오픈 실패
		return false;
	...
	if( fp_ArchitectureFile == nullptr )
	{
		return false;
	}
	fprintf_s( fp_ArchitectureFile, "%s,%s,%s,%s,%s,%s\n", "layer_name", "layer_type", "neuron_size", "activation_function_type", "loss_function_type", "optimizer_type" );

	
	for( int layerCnt = 0; layerCnt < (int)Model.size(); layerCnt++ )
	{
		if( Model[ layerCnt ] == nullptr )
		{
			fpCloseResult = NN_File_Close( fp_ArchitectureFile );
			if( fpCloseResult == false )
			{
				return false;
			}

			fp_ArchitectureFile = nullptr;

			return false;
		}

		layerName = Model[ layerCnt ]->BL_Get_BaseLayer_Name();
		...
		// 모델 아키텍처 저장
		layerType = Model[ layerCnt ]->BL_Get_BaseLayer_Type();
		neuronSize = Model[ layerCnt ]->BL_Get_NeuronSize();
		activationFunctionType = Model[ layerCnt ]->BL_Get_Activation_Type();
		...
		fprintf_s( fp_ArchitectureFile, "%s,%d,%d,%d\n", layerName.c_str(), layerType, neuronSize, activationFunctionType );


		// 가장 마지막 레이어인 출력 레이어는 weight와 bias가 없기 때문에 저장하지 않음.
		if( Model[ layerCnt ] == Model.back() )
		{
			break;
		}


		strcpy_s( weightSaveAddr, sizeof( weightSaveAddr ), this->m_modelSave_Directory_Addr );
		strcat_s( weightSaveAddr, "/" );
		strcat_s( weightSaveAddr, layerName.c_str() );
		strcat_s( weightSaveAddr, "_weight.txt" );

		strcpy_s( biasSaveAddr, sizeof( biasSaveAddr ), this->m_modelSave_Directory_Addr );
		strcat_s( biasSaveAddr, "/" );
		strcat_s( biasSaveAddr, layerName.c_str() );
		strcat_s( biasSaveAddr, "_bias.txt" );
		...
		// Weight 저장
		fopenResult = fopen_s( &fp_WeightFile, weightSaveAddr, "w" );
		errnoCheck = Errno_Checker( fopenResult );
		if( errnoCheck == false )
		{
			fpCloseResult = NN_File_Close( fp_ArchitectureFile );
		...
		weightSize = Model[ layerCnt ]->BL_Get_WeightSize();
		if( weightSize == 0 )
		{
			fpCloseResult = NN_File_Close( fp_ArchitectureFile );
			if( fpCloseResult == false )
			...
			fpCloseResult = NN_File_Close( fp_WeightFile );
			if( fpCloseResult == false )
			{
				return false;
			}

			fp_WeightFile = nullptr;
		...
		{
			weightValue = Model[ layerCnt ]->BL_Get_Weight( i );

			if( fp_WeightFile == nullptr )
			{
				fpCloseResult = NN_File_Close( fp_ArchitectureFile );
				if( fpCloseResult == false )
				{
			...
			fprintf_s( fp_WeightFile, "%f\n", weightValue );

			/*
			string str;

			weightValue = Model[ layerCnt ]->BL_Get_Weight( i );
			str = to_string( weightValue );
			str += "\n";
			...
			fprintf_s( fp_WeightFile, str.c_str() );
			*/
		}
		...
		// Bias 저장
		fopenResult = fopen_s( &fp_BiasFile, biasSaveAddr, "w" );
		errnoCheck = Errno_Checker( fopenResult );
		if( errnoCheck == false )
		{
			fpCloseResult = NN_File_Close( fp_ArchitectureFile );
		...
		biasSize = Model[ layerCnt ]->BL_Get_BiasSize();
		if( biasSize == 0 )
		{
			fpCloseResult = NN_File_Close( fp_ArchitectureFile );
			if( fpCloseResult == false )
			...
			fpCloseResult = NN_File_Close( fp_BiasFile );
			if( fpCloseResult == false )
			{
				return false;
			}
		...
		{
			biasValue = Model[ layerCnt ]->BL_Get_Bias( i );

			if( fp_BiasFile == nullptr )
			{
				fpCloseResult = NN_File_Close( fp_ArchitectureFile );
				if( fpCloseResult == false )
				{
			...
			fprintf_s( fp_BiasFile, "%f\n", biasValue );

			/*
			string str;

			biasValue = Model[ layerCnt ]->BL_Get_Bias( i );
			str = to_string( biasValue );
			str += "\n";
			...
			fprintf_s( fp_BiasFile, str.c_str() );
			*/
		}
	...
	lossFunctionType = NN_Get_LossFunction_Type();
	optimizerType = NN_Get_Optimizer_Type();
	fprintf_s( fp_ArchitectureFile, ",,,,%d,%d", lossFunctionType, optimizerType );
...
bool NeuralNetwork::NN_Add_Layer( E_LayerType layerType, int row, int column, E_ActivationType activationType, string structureName )
{
	...
	{
		::gLog_ErrorLog.LS_AddLog( _T( "Layer is maximun" ) );
...
bool NeuralNetwork::NN_File_Close( FILE *fp_filePtr )
{
	if( fp_filePtr == nullptr )
	{
		::gLog_ErrorLog.LS_AddLog( _T( "File close fail - NeuralNetwork.cpp" ) );
...
void NeuralNetwork::Critical_Section_Init()
{
...
void NeuralNetwork::Critical_Section_Delete()
{
